...
def unbag_fly(fly_num):
    print(fly_num)
    resample_freq = 50  # a bit faster than the Ca-cam will get wing aliasing
    ...
    # from cvbridge3 import CvBridge, CvBridgeError
    cv_bridge = CvBridge()
    ...
    if len(ca_cam_right_tstamps) > 0:
        ca_cam_tstamps = ca_cam_right_tstamps
        ca_cam_ros_tstamps = ca_cam_right_ros_tstamps
    elif len(ca_cam_left_tstamps) > 0:
        ca_cam_tstamps = ca_cam_left_tstamps
        ca_cam_ros_tstamps = ca_cam_left_ros_tstamps
    else:
        print('No CA signal to align to')
        return
    
    criterion =  np.argwhere(kine_cam1_tstamps < ca_cam_tstamps[0])
    if len(criterion) > 0:
    ...
    first_ca_tstamp_ros = ca_cam_ros_tstamps[0]
    ...
    ca_cam_tstamps = np.array(ca_cam_tstamps)
    ca_cam_tstamps -= first_kine_tstamp_ros
    ...
    last_ca_tstamp_exp = ca_cam_tstamps[-1]
    ...
    def get_data(t):
        ...
        ca_cam_idx = choose_last(np.argwhere(ca_cam_tstamps<t))
        ...
        #deal with the fact that we might lack left or right cam
        if len(ca_cam_right_imgs) > 0:
            ca_cam_right_out = ca_cam_right_imgs[ca_cam_idx]
        else:
            ca_cam_right_out = np.array([])
        
        if len(ca_cam_left_imgs) > 0:
            ca_cam_left_out = ca_cam_left_imgs[ca_cam_idx]
        else:
            ca_cam_left_out = np.array([])

        return {'ca_cam_right':ca_cam_right_out,
                'ca_cam_left':ca_cam_left_out,
