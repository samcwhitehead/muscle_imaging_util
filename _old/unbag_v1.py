...
class BagToHDF5:
    # ----------------------------------------------------------------------
    def __init__(self, fly_db_path=FLY_DB_PATH, fly_path=None):
        """
    ...
    def get_file_paths(self, bagfile=None):
        ...
        h5_fn = 'converted_' + self.bagfile_name[:-4] + '.hdf5'
    ...
    def read_kinefly_data(self, topic_name):
        ...
        self.create_dataset('flystate_seq', data=seq)
        self.create_dataset('flystate_langles', data=langles)
        self.create_dataset('flystate_lgrandients', data=lgradients)
        self.create_dataset('flystate_lintensity', data=lintensity)
        self.create_dataset('flystate_rangles', data=rangles)
        self.create_dataset('flystate_rgradients', data=rgradients)
        self.create_dataset('flystate_rintensity', data=rintensity)
    ...
    def read_general_msg_data(self, topic_name):
        ...
        # write DAQ messages to hdf5
        print('writing %s data for %s' % (topic_name, self.fly_id_str))
        self.h5f.create_dataset(str(topic_name[1:]) + '/ros_tstamps', data=ros_tstamps)
        self.h5f.create_dataset(str(topic_name[1:]) + '/msgs', data=msg_data)
    ...
    def read_metadata(self, topic_name='/exp_scripts/exp_metadata'):
        ...
        mtd_keys = ['git_SHA', 'exp_description', 'fly_genotype', 'fly_genotype', 'genotype_nickname', 'script_code']
    ...
    def read_cam_data(self, topic_name):
        ...
        with h5py.File(save_path, 'w') as f:
            f.create_dataset('cam_ros_tstamps', data=ros_tstamps)
            f.create_dataset('cam_tstamps', data=tstamps)
            f.create_dataset('cam_imgs', data=np.asarray(imgs), compression="gzip")
    ...
    def read_rframe_data(self, topic_name):
        ...
        print('writing %s data for %s' % (topic_name, self.fly_id_str))
        with h5py.File(save_path, 'w') as f:
            f.create_dataset('rframe_ros_tstamps', data=ros_tstamps)
            f.create_dataset('ref_frame_components', data=ref_frame_components)
            f.create_dataset('ref_frame_p', data=np.array(ref_frame_p))
            f.create_dataset('ref_frame_a1', data=np.array(ref_frame_a1))
            f.create_dataset('ref_frame_a2', data=np.array(ref_frame_a2))
            f.create_dataset('ref_frame_A', data=np.array(ref_frame_A))
            f.create_dataset('ref_frame_A_inv', data=np.array(ref_frame_A_inv))
